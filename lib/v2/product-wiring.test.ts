...
describe("comparison eligibility and matching", () => {
  ...
  it("uses immutable creation order for same-date review pairs", () => { const first = review("first", "2026-07-17", [old]); const second = { ...review("second", "2026-07-17", [current]), createdAt: "2026-07-17T11:00:00.000Z" }; expect(latestEligiblePair([first, second])).toEqual({ previous: first, current: second }); });
  it("reports exact eligibility failures", () => expect(comparisonEligibility(latest, previous).failures).toContain("The previous review must be earlier by review date or saved-snapshot creation order."));
