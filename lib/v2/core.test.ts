...
describe("v2 foundation", () => {
  it("defines four primary packs and a recruitment example pack", () => { expect(Object.keys(verticalPacks)).toEqual(["general", "interiors", "agency", "saas", "recruitment"]); expect(Object.values(verticalPacks).filter((pack) => pack.category === "built-in")).toHaveLength(4); expect(verticalPacks.recruitment.category).toBe("example"); for (const pack of Object.values(verticalPacks)) { expect(pack.fields.some((field) => field.importance === "required")).toBe(true); expect(pack.stages.some((stage) => stage.lifecycle === "won")).toBe(true); expect(pack.samples.current.length).toBeGreaterThan(0); } });
...
describe("attention and actions", () => {
  ...
  it("scores explainably with conservative monetary exposure", () => { const assessment = assessOpportunity(opportunity, "2026-07-17", verticalPacks.general); expect(assessment.score).toBeGreaterThanOrEqual(50); expect(assessment.rules.map((rule) => rule.id)).toContain("overdue-follow-up"); expect(assessment.valueAtRisk).toBe(60000); expect(assessment.valueAtRiskFactor).toBe(.6); expect(assessOpportunity({ ...opportunity, value: undefined }, "2026-07-17", verticalPacks.general).valueAtRisk).toBeUndefined(); });
